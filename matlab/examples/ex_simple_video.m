...
noise = repmat( randn(size(V_ref,1),size(V_ref,2),size(V_ref,3))*max_v*N_amplitude, [1 1 1 size(V_ref,4)] );
V_static_noise = uint16( clamp( single(V_ref) + noise, 0, 2^16-1 ) ); % Static Gaussian noise
