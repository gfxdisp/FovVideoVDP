...
"""
class fvvdp:
    def __init__(self, display_name="standard_4k", display_photometry=None, display_geometry=None, color_space="sRGB", foveated=False, heatmap=None, quiet=False, device=None, display_models=None, use_checkpoints=False):
        self.quiet = quiet
    ...
    def predict_video_source(self, vid_source, fixation_point=None, frame_padding="replicate"):
        ...
        if self.debug: self.tb.verify_against_matlab(self.Q_per_ch, 'Q_per_ch_data', self.device, file='Q_per_ch', tolerance = 0.1, relative=True, verbose=True)
    ...
    stored to limit GPU memory used. If they are needed for autograd, they will be dynamically
    recomputed during backpropagation.

    This is essentially the classic "memory VS compute" trade-off
    Reference: Chen, Tianqi, et al. "Training deep nets with sublinear memory cost." arXiv preprint arXiv:1604.06174 (2016).
    '''
    def process_frame(self, ff, R, vid_sz, temp_ch, fixation_point, heatmap):
        ...
        if self.Q_per_ch is None: self.Q_per_ch = torch.zeros((len(B_bands)-1, 2, N_frames), device=self.device)
