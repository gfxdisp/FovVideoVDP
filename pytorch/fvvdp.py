...
class fvvdp_video_source_dm( fvvdp_video_source ):
    ...
    # test_video, reference video - tensor of the size:
    #   [height width 3] - for colour images
    #   [height width 3 N] - for colour video of N frames
    #   [height width N] - for grayscale video of N frames
...
class fvvdp:
    def __init__(self, display_name="standard_4k", display_photometry=None, display_geometry=None, color_space="rec2020", foveated=False, heatmap=None, quiet=False):
        self.quiet = quiet
        self.foveated = foveated
        self.heatmap = heatmap

        if display_photometry is None:
            display_photometry = fvvdp_display_photometry.load(display_name)

        if display_geometry is None:
            display_geometry = fvvdp_display_geometry.load(display_name)

    def predict(self, test_cont, reference_cont):
        pass
